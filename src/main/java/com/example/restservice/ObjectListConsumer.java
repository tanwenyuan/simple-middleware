...
public class ObjectListConsumer {
...
//        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        ...
        Consumer<String, List<User>> consumer = new KafkaConsumer<>(props);
        consumer.subscribe(Arrays.asList("kafeidou"));

        while (true) {
            Thread.sleep(3000L);
            ConsumerRecords<String, List<User>> records = consumer.poll(1000);
            for (ConsumerRecord<String, List<User>> record : records) {
